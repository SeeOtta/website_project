window.addEventListener("load", function(){ // Wait for the page to load
    ...
    const form = document.querySelector(".fcf-form-id") // Get the form

    form.addEventListener("submit", function (event){
        event.preventDefault() // Prevent the default action of the form
        let fields = document.querySelectorAll(".fcf-form-id .fcf-form-control") // Get all the fields
        let valid = true
        ...
        if(valid){ // If all the fields are valid
            ...
            grecaptcha.ready(function() { // Wait for the recaptcha to be ready
                grecaptcha
                    .execute("6LcyTwojAAAAAFJGM6GBu7jzEu27EXVQILXWtuef", {
                        action: "contact"
                    ...
                    .then(function(token){
                        ...
                        fetch("/send.php", {
                            ...
                            .then((response) => {
                                ...
                                document.querySelector("#alert").classList.add("")
                                window.location.replace("/pagina3.html") // Redirect to the thanks page
                            })
                    })
            })
        }
